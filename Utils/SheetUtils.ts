export const GetSingleFolder = (folderName: string): GoogleAppsScript.Drive.Folder => {
...
export const GetSingleFile = (parentFolder: GoogleAppsScript.Drive.Folder, fileName: string): GoogleAppsScript.Drive.File => {
...
export const GetSingleSheet = (sheet: GoogleAppsScript.Spreadsheet.Spreadsheet, sheetName: string): GoogleAppsScript.Spreadsheet.Sheet => {
...
export const GetSingleRow = (sheet: GoogleAppsScript.Spreadsheet.Sheet, row: number): any[] => {
...
export const IndexToHeader = (sheet: GoogleAppsScript.Spreadsheet.Sheet): { [key: string]: number } => {
    var firstRow = GetSingleRow(sheet, 1);
    var result = {};
    firstRow.forEach((value, index) => result[value.toString().toLowerCase()] = index);
    return result;
}
export const FindColumnIndex = (lut: { [key: string]: number }, pattern: RegExp): number => {
